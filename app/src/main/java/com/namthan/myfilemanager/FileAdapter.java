...
import com.namthan.myfilemanager.databinding.FileItemRowBinding;
...
public class FileAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public FileAdapter(List<FileModel> listFile, OnClickListener listener) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof FileViewHolder) {
            ((FileViewHolder) holder).bind(listFile.get(position));
        } else if (holder instanceof DirectoryViewHolder) {
            ((DirectoryViewHolder) holder).bind(listFile.get(position));
        }
    ...
    public int getItemViewType(int position) {
        if (listFile.get(position).getType() == FileType.DIRECTORY) {
            return 0;
        }
        return 1;
    }
    ...
    public static class DirectoryViewHolder extends RecyclerView.ViewHolder implements View.OnCreateContextMenuListener {
        ...
        public void bind(FileModel fileModel) {
            binding.fileNameTv.setText(fileModel.getFileName());
            if (fileModel.getType() == FileType.DIRECTORY) {
                binding.iconIv.setBackgroundResource(
                        R.drawable.folder);
            } else {
                binding.iconIv.setBackgroundResource(R.drawable.file);
            }
